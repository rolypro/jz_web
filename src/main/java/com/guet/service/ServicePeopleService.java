...
import com.guet.dao.ServicePeopleDao;
...
@Service("servicePeopleService")
public class ServicePeopleService{

	@Autowired
	private ServicePeopleDao dao;
	@Autowired
	private EvaluateDao evaluateDao;
	
	
	//查停用 查启用 查待审核
	public List<ServicePeople> findState(int state){
		return dao.findState(state);
	}
	
	//根据id查询
	public ServicePeople findById(int id) {
		return dao.findById(id);		
	}
	//根据uId查询
	public ServicePeople findByUid(int uId) {
		return dao.findByUid(uId);
	}
	
	//模糊查找
	public List<ServicePeople> findDim(String str){
		return dao.findDim(str);
	}
	
	public List<ServicePeople> findByType(String city,String server,String edu){
		return dao.findByType(city, server, edu);
	}
