                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
	                                ...
	                                <c:forEach var="sc" items="${scs }" varStatus="status">
										<tr id="${status.count }">
											<td>${sc.id }</td>
											<td>${sc.uId }</td>
											<td >${sc.name }</td>											
											<td >${sc.phone }</td>
											<td >${sc.serviceCity }</td>
											<td >${sc.state }</td>											
											<td >	
												<input class="btn btn-primary btn-xs" name="state" id="1" type="button" value="同意">
												<input class="btn btn-primary btn-xs" name="state" id="3" type="button" value="驳回">
												
												<input class="btn btn-primary btn-xs" name="check" id="${sc.id }" type="button" value="查看">
												<input class="btn btn-primary  btn-xs" name="del" id="${sc.id }" type="button" value="删除">
											</td>
										</tr>
									</c:forEach> 
                                </tbody>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("input[name='check']").click(function(){
				id = this.id;
				window.location.href="serviceCompany.do?method=findById&&id="+id; 
			});
			...
			$("input[name='state']").click(function(){
				...
				window.location.href="serviceCompany.do?method=updateState&&id="+id+"&&state="+state; 
			
			});
			
			$("input[name='del']").click(function(){
				id = this.id;
				window.location.href="serviceCompany.do?method=delete&&id="+id; 
			});
