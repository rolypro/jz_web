                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
	                                ...
	                                <c:forEach var="sm" items="${m.subMessage }" varStatus="status">
										<tr id="${status.count }">
											<td>${sm.id }</td>
											<td>${sm.uId }</td>
											<td >${sm.name }</td>
											<td >${sm.content }</td>
											<td ><fmt:formatDate value="${sm.createTime }" pattern="yyyy-MM-dd"/></td>
											<td >
												<input class="btn btn-primary  btn-xs" name="del" id="${sm.id }" type="button" value="删除">
											</td>
										</tr>
									</c:forEach> 
                                </tbody>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("input[name='mb']").click(function(){
				id = this.id;
				window.location.href="news.do?method=updateView&&id="+id; 
			});
			
			$("input[name='del']").click(function(){
				id = this.id;
				window.location.href="message.do?method=deleteSub&&id="+id; 
			});
