                                ...
                                <tbody>
	                                ...
	                                <c:forEach var="jo" items="${jos }" varStatus="status">
										<tr id="${status.count }">
											...
											<td>
												<c:if test="${jo.state ==6}">已完成</c:if>
												<c:if test="${jo.state==7}">已评价</c:if>
											</td>
											<td >
												<c:if test="${jo.state == 6}"><a href="jzOrder.do?method=reviewView&&id=${jo.id }"><input class="btn btn-primary  btn-xs" name="del" id="${jo.id }" type="button" value="评价订单"></a></c:if>
												<c:if test="${jo.state == 7}"><input class="btn btn-primary  btn-xs" name="del" id="${jo.id }" type="button" value="删除订单"></c:if>
											</td>
										</tr>
									</c:forEach> 
                                </tbody>
